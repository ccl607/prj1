...
<!--                 <div class="darkblue-panel pn">
  ...
  <script type="text/javascript">
    ...
    function getChartsData() {
		$.ajax({
		   url : "/group4erp/getMainCharts.do"
		   , type: "get"
	...
	function gridCharts(data) {
		...
		/* $.each(data.sellingStat, function(idx, ele) {
			html += "<div class=\"bar\">";
			html += "<div class=\"title\"><font style='font-size:12pt; font-weight:bold'>" + ele.title + "</font></div>";
			html += "<div class=\"value tooltips\" data-original-title=\"" + 8.500 + "\" data-toggle=\"tooltip\" data-placement=\"top\">" + 85% + "</div>";
			html += "</div>";
		}); */
		...
		/* 
	  ,{
		    value: data.ageStat[5].cnt,
		    color: 'blue'
	   }

 
	  ,{
	    value: data.ageStat[6].cnt,
	    color: 'green'
	  }*/
		  ];
...
/* 		var lastyearEvnt ="<table border='1' cellpadding='0' style='cellpadding:0; margin:0;' width='100%'><tr><td><h5>"+ data.eventStat[0].title+"&nbsp;:&nbsp;";
		...
		var genderData = [{
		    value: data.genderStat[0].cnt,
		    color: "#FF6B6B"
		  },
		  {
		    value: data.genderStat[1].cnt,
		    color: "#1c9ca7"
		  }
		];

		var genderText = "<h2><font style='font-size:20pt'>"+data.genderStat[0].cnt+"%</font>";
		genderText += "<font style='font-size:20pt; color:darkgray;'>/</font>";
		genderText += "<font style='font-size:20pt; color:#1c9ca7;'>"+data.genderStat[1].cnt+"%</font></h2>"; 
...
/* 		var evnt_text = "<p><b><font style='font-size:12pt;'>어제 도서계약 건수 : "+ data.reg_cnt+ "건</font></p>";
		...
		/* var genderDonut = Morris.Donut({
			...
			data: [
				{label: data.genderStat[0].title, value: data.genderStat[0].cnt },
				{label: data.genderStat[1].title, value: data.genderStat[1].cnt }
			],
			colors: ['#f68275', '#1c9ca7'],
			formatter: function (y) { return y + "%" }
			//resize : true
		});
		$("#gender-donut:nth-child(1)").css('margin-left','25px');
		//$("#gender-donut:nth-child(1)").css('margin-bottom','20px');
		genderDonut.redraw(); */
	}
